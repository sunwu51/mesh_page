...
fs.writeFileSync(reduPath,fs.readFileSync('./template/reducer.js').toString()
    ...
    .replace(/\$get\.path\$/g,conf.url.get.path).replace(/\$get\.method\$/g,conf.url.get.method)
    .replace(/\$add\.path\$/g,conf.url.add.path).replace(/\$add\.method\$/g,conf.url.add.method)
    .replace(/\$delete\.path\$/g,conf.url.delete.path).replace(/\$delete\.method\$/g,conf.url.delete.method)
    .replace(/\$update\.path\$/g,conf.url.update.path).replace(/\$update\.method\$/g,conf.url.update.method))
    ;
